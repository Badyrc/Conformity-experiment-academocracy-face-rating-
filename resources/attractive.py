...
def setupData(expInfo, dataDir=None):
    ...
    if dataDir is None:
        dataDir = _thisDir
    filename = u'data/%s_%s_%s' % (expInfo['姓名（拼音）'], expName, expInfo['date'])
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\ti\\Desktop\\REACTANCE SCT AND CONFORMITY\\PSYCHOPY\\attractive-rating-master\\attractive.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text='Welcome to this experiment!\n\nThis experiment requires you to rate the attractiveness of faces.\n\nFirst, a face will appear in the center of the screen.\n\nAfter the face disappears, rate it from 1 to 7 using the + icon.\n\n1 = least attractive; 7 = most attractive.\n\nWe hope you can rate it as quickly as possible without much thought.\n\nAnd keep your gaze focused on the center of the screen throughout the experiment.\n\nIf you are ready, please press the spacebar to begin.',
        font='Arial',
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='The practice phase is over; the formal experiment will begin next!\n\nAgain, please remember to score as quickly as possible without overthinking.\n\nAnd keep your gaze centered on the screen throughout the entire experiment.\n\nIf you are ready, please press the spacebar to begin the formal experiment.',
        font='Arial',
    ...
    fix2 = visual.TextStim(win=win, name='fix2',
        ...
        languageStyle='LTR',
        depth=0.0);
    allrating = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    for thisPractice_trial in practice_trials:
        ...
        practice_rating = data.Routine(
            ...
            components=[fix1, practiceRating],
        )
        ...
        while continueRoutine:
            ...
            if practiceRating.status == STARTED and not waitOnFlip:
                theseKeys = practiceRating.getKeys(keyList=['1','2','3','4','5','6','7'], ignoreKeys=["escape"], waitRelease=False)
    ...
    for thisTrial in trials:
        ...
        rating = data.Routine(
            name='rating',
            components=[fix2, allrating],
        )
        rating.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # create starting attributes for allrating
        allrating.keys = []
        allrating.rt = []
        _allrating_allKeys = []
        ...
        while continueRoutine:
            ...
            # if allrating is starting this frame...
            if allrating.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                allrating.frameNStart = frameN  # exact frame index
                allrating.tStart = t  # local t and not account for scr refresh
                allrating.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(allrating, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'allrating.started')
                # update status
                allrating.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(allrating.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(allrating.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if allrating.status == STARTED and not waitOnFlip:
                theseKeys = allrating.getKeys(keyList=['1','2','3','4','5','6','7'], ignoreKeys=["escape"], waitRelease=False)
                _allrating_allKeys.extend(theseKeys)
                if len(_allrating_allKeys):
                    allrating.keys = _allrating_allKeys[-1].name  # just the last key pressed
                    allrating.rt = _allrating_allKeys[-1].rt
                    allrating.duration = _allrating_allKeys[-1].duration
                    # a response ends the routine
                    continueRoutine = False
        ...
        # check responses
        if allrating.keys in ['', [], None]:  # No response was made
            allrating.keys = None
        trials.addData('allrating.keys',allrating.keys)
        if allrating.keys != None:  # we had a response
            trials.addData('allrating.rt', allrating.rt)
            trials.addData('allrating.duration', allrating.duration)
