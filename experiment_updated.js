...
(async function() {
  ...
  let savedFilename = '';
  const EMAIL_TO = 'batoturbiev@yandex.ru';
  const RESPONSES_REQUIRED = false;
  function isResponseRequired() { return RESPONSES_REQUIRED; }
  function buildMailtoLink(filename, participantId, stageLabel) {
    const subject = encodeURIComponent('Данные исследования — ' + stageLabel + ' — ' + (participantId || 'participant'));
    const body = encodeURIComponent('Здравствуйте!\n\nПожалуйста, прикрепите к этому письму скачанный файл ' + stageLabel + ': ' + (filename || 'data.csv') + '.\n\nСпасибо!');
  ...
  timeline.push({
    ...
    stimulus: instructionBlock(`
      <div style="max-width:900px; margin:0 auto;">
        <div style="text-align:center; font-size:30px; font-weight:700; margin-bottom:20px;">Перерыв 5 минут</div>
        <p>Перед началом следующего задания, пожалуйста, сделайте пятиминутный перерыв. Это поможет снизить влияние предыдущей части исследования на следующую.</p>
        <p>Ниже вы увидите таймер. Пока он идёт, можно просто отдохнуть или поиграть в небольшую расслабляющую мини-игру: нажимайте на появляющиеся пузырьки.</p>
        <div id="break-timer" style="font-size:42px; font-weight:700; text-align:center; margin:24px 0;">05:00</div>
        <div id="bubble-game" style="position:relative; width:min(720px, 90vw); height:320px; margin:0 auto; border:1px solid #ccd; border-radius:18px; overflow:hidden; background:linear-gradient(180deg, #f7fbff 0%, #eef7ff 100%);"></div>
        <p style="margin-top:14px; text-align:center;">Когда таймер закончится, кнопка продолжения станет доступной.</p>
      </div>
    `),
    choices: ['Перейти к следующему заданию'],
    data: { phase: 'five_minute_break' },
    on_load: function() {
      const button = document.querySelector('#jspsych-html-button-response-button-0');
      if (button) button.disabled = true;

      const timerEl = document.getElementById('break-timer');
      const area = document.getElementById('bubble-game');
      let remaining = 300;
      ...
      function spawnBubble() {
        ...
        bubble.style.left = Math.random() * Math.max(area.clientWidth - size, 10) + 'px';
        bubble.style.top = area.clientHeight + 'px';
        bubble.style.background = 'radial-gradient(circle at 30% 30%, rgba(255,255,255,0.95) 0%, rgba(180,220,255,0.9) 45%, rgba(120,180,255,0.75) 100%)';
        bubble.style.boxShadow = '0 4px 14px rgba(100,140,220,0.25)';
        ...
        let topPos = area.clientHeight;
        const speed = 0.35 + Math.random() * 0.6;
        const drift = (Math.random() - 0.5) * 0.5;
        let leftPos = parseFloat(bubble.style.left);
        ...
        function animate() {
          ...
          if (topPos < -size || !document.body.contains(bubble)) {
            bubble.remove();
            return;
          }
      ...
      const bubbleSpawner = setInterval(spawnBubble, 1200);
      for (let i = 0; i < 6; i++) {
        setTimeout(spawnBubble, i * 250);
      }

      area.dataset.intervalId = String(bubbleSpawner);
      area.dataset.timerIntervalId = String(intervalId);
    },
    on_finish: function() {
      const area = document.getElementById('bubble-game');
      if (area && area.dataset.intervalId) {
        clearInterval(Number(area.dataset.intervalId));
      }
      if (area && area.dataset.timerIntervalId) {
        clearInterval(Number(area.dataset.timerIntervalId));
      }
    }
  });
  ...
  timeline.push({
    ...
    stimulus: instructionBlock(`
      ...
      <p>Текстовое задание, которое вы выполнили в первой части исследования, было оценено руководителем проекта.</p>
      <p>Ниже будет показано его(её) личное и честное мнение о выполненной вами работе.</p>
  ...
  timeline.push({
    ...
    func: function() {
      ...
      savedFilename = `${pid}_attractive_${stamp}.csv`;
      downloadText(savedFilename, csv, 'text/csv');
  ...
  timeline.push({
    ...
    stimulus: function() {
      const pid = jsPsych.data.get().values()[0]?.participant_id || '001';
      const mailto = buildMailtoLink(savedFilename, pid, 'часть 2');
      return instructionBlock(`
        <div style="max-width:900px; margin:0 auto;">
          <div style="text-align:center; font-size:28px; font-weight:700; margin-bottom:20px;">Файл второй части сохранён</div>
          <p>Если вы хотите сразу отправить файл второй части исследователю, откройте письмо по ссылке ниже и прикрепите к нему скачанный CSV-файл.</p>
          <p style="text-align:center; margin:22px 0;"><a href="${mailto}" target="_blank" rel="noopener noreferrer">Открыть письмо для отправки файла второй части</a></p>
          <p>После этого нажмите кнопку ниже, чтобы прочитать заключительное сообщение.</p>
        </div>
      `);
    },
    choices: ['Перейти к заключительному сообщению'],
    data: { phase: 'email_prompt_stage2' }
  });
...
Во время выполнения текстового задания некоторые участники
