...
async function sendStage2Results(csvText, participantId, conditionLabel) {
  const formData = new FormData();
  formData.append("access_key", WEB3FORMS_ACCESS_KEY);
  formData.append("subject", `Conformity experiment stage 2 - ${participantId}`);
...
if (incoming.participant_id || incoming.age || incoming.sex) {
  ...
  timeline.push({
    type: jsPsychHtmlKeyboardResponse,
    stimulus: `
      <div style="max-width:900px; margin:60px auto; font-family:Arial,sans-serif; text-align:center;">
        <h2>Завершение</h2>
        <p id="stage2-send-status">Результаты второй части отправляются в онлайн-хранилище.</p>
        <p>Пожалуйста, подождите несколько секунд.</p>
      </div>`,
    choices: "NO_KEYS",
    on_load: function() {
      const statusEl = document.getElementById('stage2-send-status');
      const pid = jsPsych.data.get().values()[0]?.participant_id || '001';
      const csv = jsPsych.data.get().csv();
      const conditionLabel = incoming.condition || '';

      (async function() {
        try {
          const data = await sendStage2Results(csv, pid, conditionLabel);
          if (data && data.success) {
            statusEl.textContent = "Результаты второй части успешно отправлены.";
          } else {
            statusEl.textContent = "Не удалось подтвердить отправку. Локальная копия будет сохранена.";
          }
        } catch (error) {
          statusEl.textContent = "Возникла ошибка отправки. Локальная копия будет сохранена.";
        } finally {
          setTimeout(function() {
            jsPsych.finishTrial();
          }, 1200);
  ...
  timeline.push({
    ...
    stimulus: `
      <div class="jspsych-content" style="max-width:900px; margin:60px auto; font-family:Arial,sans-serif; text-align:center;">
        ...
        <p>Вы завершили исследование.</p>
        <p>Локальная копия данных также будет сохранена на это устройство в качестве резервной.</p>
        <p><strong>Нажмите пробел, чтобы завершить.</strong></p>
      </div>`,
    choices: [' ']
  });
