...
(async function() {
  ...
  timeline.push({
    ...
    func: function() {
      ...
      stage2Filename = `${pid}_stage2_${stamp}.csv`;
      const csv = jsPsych.data.get().csv();
      downloadText(stage2Filename, csv, 'text/csv');
      stage2Mailto = 'mailto:batoturbiev@yandex.ru'
        + '?subject=' + encodeURIComponent('Stage 2 data: ' + pid)
        + '&body=' + encodeURIComponent(
          'Здравствуйте!\n\nВо вложении файл второй части исследования.\n' +
          'ID участника: ' + pid + '\n' +
          'Файл: ' + stage2Filename + '\n\n' +
          'Пожалуйста, прикрепите скачанный CSV-файл к этому письму и отправьте его.'
        );
      jsPsych.data.addProperties({
        stage2_filename: stage2Filename
      });
    },
    data: { phase: 'save_stage2_data' }
  });

  timeline.push({
    type: jsPsychHtmlKeyboardResponse,
    stimulus: `
      <div style="max-width:820px; margin:50px auto; font-family:Arial,sans-serif; text-align:center; line-height:1.7;">
        <h3>Файл с результатами второй части сохранён</h3>
        <p>Сейчас можно открыть черновик письма и отправить исследователю скачанный файл.</p>
        <p><b>Важно:</b> файл нужно прикрепить к письму вручную.</p>
        <p><a id="stage2MailLink" href="#">Открыть черновик письма</a></p>
        <p style="margin-top:20px;"><button id="afterMailContinue" style="padding:10px 18px; font-size:16px;">Продолжить</button></p>
      </div>
    `,
    choices: "NO_KEYS",
    on_load: function() {
      document.getElementById('stage2MailLink').onclick = function(e) {
        e.preventDefault();
        window.location.href = stage2Mailto;
      };
      document.getElementById('afterMailContinue').onclick = function() {
        jsPsych.finishTrial();
      };
    },
