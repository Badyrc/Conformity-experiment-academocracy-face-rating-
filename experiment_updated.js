...
(async function() {
  ...
  const SUSPICION_SELECT_REQUIRED = true;
  ...
  function buildStage2FormattedRow() {
    ...
    const row = {
      ...
      age: first.age || '',
      sex: first.sex || '',
      upstream_condition: first.upstream_condition || '',
      feedback_condition: first.feedback_condition || assignedFeedback.key,
      feedback_text: first.feedback_text || assignedFeedback.text
    };

    const messageQ = jsPsych.data.get().filter({ phase: 'message_questionnaire' }).values()[0];
    if (messageQ && messageQ.response) {
      Object.keys(messageQ.response).forEach(key => {
        row['message_' + key] = messageQ.response[key];
      });
    }

    const suspicionQ = jsPsych.data.get().filter({ phase: 'suspicion_questionnaire' }).values()[0];
    if (suspicionQ && suspicionQ.response) {
      Object.keys(suspicionQ.response).forEach(key => {
        row['suspicion_' + key] = suspicionQ.response[key];
      });
    }

    const mainRatings = jsPsych.data.get().filter({ phase: 'main_rating' }).values();
    mainRatings.forEach((trial, idx) => {
      const n = idx + 1;
  ...
  timeline.push({
    ...
    func: function() {
      ...
      const formattedCsv = toCSVFromRows([buildStage2FormattedRow()]);
      downloadText(`${pid}_stage2_raw_${stamp}.csv`, rawCsv, 'text/csv;charset=utf-8;');
      setTimeout(() => {
        downloadText(`${pid}_stage2_formatted_${stamp}.csv`, formattedCsv, 'text/csv;charset=utf-8;');
      }, 250);
    },
    data: { phase: 'download_stage2_data' }
  });
